...
class TestTools:
    ...
    def test_search_cds_documents_success(self, mock_client_class):
        ...
        mock_search_response = CDSSearchResponse(total=1, records=[mock_record])

        mock_client.search.return_value = mock_search_response

        # Call the tool
        result = search_cds_documents(
            query="higgs boson", experiment="ATLAS", doc_type="Article", size=5
        )
    ...
    def test_get_cds_document_files_success(self, mock_client_class):
        ...
        assert (
            file1["download_url"] == "https://cds.cern.ch/record/123456/files/paper.pdf"
        )
    ...
    def test_get_cds_document_files_error(self, mock_client_class):
        ...
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        mock_client.get_record_files.side_effect = CDSClientError(
